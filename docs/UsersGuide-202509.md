...
**バージョン**: 1.0
**作成日**: 2025年9月14日
**基準日**: 2025年9月12日をVersion 1.0とする
**対象**: 一般利用者向け操作ガイド

## はじめに

PointGPSは、国土地理院地図上でGPSポイントを簡単に管理できるWebアプリケーションです。Excel専用化により30-50倍の大幅高速化を実現し、直感的な操作でGPSポイントの編集・管理が行えます。
...
### 重要なお知らせ
- **Excel専用化**: データの読み込み・出力はExcel(.xlsx)形式のみ対応
- **大幅高速化**: 従来比30-50倍の処理速度向上を実現
- **動的メッセージ表示**: ポイント操作時に具体的なポイントID付きメッセージが表示
- **1000行制限**: Excelファイル読み込み時、最大1000行まで処理
...
| バージョン | 日付 | 主な変更内容 |
|-----------|------|-------------|
| 1.0 | 2025-09-12 | Version 1.0として設定、基本機能完成版 |
